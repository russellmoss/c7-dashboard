...
async function fetchPaginatedData(endpoint, queryParams = {}) {
    ...
    while (continueLooping && page <= MAX_PAGES) {
        ...
        while (currentTry <= MAX_RETRIES && !success) {
            try {
                ...
                if (page % 10 === 0) {
                    log.cyan(`   📊 Progress: Page ${page}, ${allItems.length} items fetched so far...`);
                } else if (page <= 3) {
                ...
                if (items.length > 0) {
                    ...
                    if (page <= 3 || page % 10 === 0) {
                        log.darkGray(`   - Page ${page}: Fetched ${items.length} items. (Total: ${allItems.length})`);
                    }
                } else {
                ...
                if (items.length < API_LIMIT) {
                    log.darkGray(`   - Reached the end of the data for this endpoint (Page ${page} had only ${items.length} items).`);
                    totalPages = page;
                    continueLooping = false;
                } else {
    ...
    log.success(`✅ Total items fetched from '${endpoint}': ${allItems.length} (from ${totalPages} pages)`);
    return allItems;
}
...
async function fetchAllDataParallel(dateRanges) {
    ...
    return { allOrders, clubMemberships, currentPeriodOrders, previousPeriodOrders };
}
...
async function fetchAllDataForAllQuarters(baseDate = new Date()) {
    ...
    quarterRangesToFetch.forEach(q => {
        log.info(`   - Q${q.quarter}: ${q.current.start.toISOString().split('T')[0]} to ${q.current.end.toISOString().split('T')[0]}`);
        log.info(`   - Q${q.quarter} Prev: ${q.previous.start.toISOString().split('T')[0]} to ${q.previous.end.toISOString().split('T')[0]}`);
    });
    
    // Execute order fetches sequentially (respecting rate limits)
    log.info(`🔄 Executing ${quarterRangesToFetch.length * 2} targeted order API calls sequentially...`);
    const allOrderResults = [];
    let callCount = 0;
    
    for (const q of quarterRangesToFetch) {
        callCount++;
        log.darkCyan(`   - Executing order fetch ${callCount}/${quarterRangesToFetch.length * 2}: Q${q.quarter} ${q.current.start.getFullYear()}...`);
        ...
        callCount++;
        log.darkCyan(`   - Executing order fetch ${callCount}/${quarterRangesToFetch.length * 2}: Q${q.quarter} ${q.previous.start.getFullYear()}...`);
    ...
    const earliestDate = Math.min(...quarterRangesToFetch.map(q => q.previous.start.getTime()));
    ...
    const fetchDuration = (Date.now() - startTime) / 1000;
    log.success(`✅ Optimized all-quarters data fetched in ${fetchDuration.toFixed(2)} seconds`);
    
    log.info(`📊 Optimized Data Summary:`);
    log.info(`   - Total API calls made: ${quarterRangesToFetch.length * 2 + 1} (${quarterRangesToFetch.length * 2} order calls + 1 club call)`);
    log.info(`   - Total unique orders: ${allOrders.length}`);
    log.info(`   - Club memberships: ${clubMemberships.length}`);
...
function getDefinitions() {
    return {
        serviceTypes: {
            tasting: "Orders containing at least one wine flight/tasting item (takes precedence over all other categories)",
            dining: "Orders containing dining/food items but no tasting items",
            byTheGlass: "Orders containing wine by the glass items but no tasting or dining items",
            retail: "All other orders (typically bottle purchases, merchandise, or other items not in the above categories)"
        },
        metrics: {
            clubConversionRate: "Calculated as (Total Customers Who Signed Up for Club / Total Non-Club and Trade Guests) × 100. This metric excludes existing club members from the calculation to normalize the conversion rate and show the true effectiveness of converting non-members into club members.",
            wineBottleConversionRate: "The percentage of guests who purchased wine bottles during their visit",
            conversionRate: "The percentage of guest experiences that resulted in wine bottle purchases",
        ...
        performanceGoals: {
            ...
            goalVariance: "The difference between actual performance and goal. Positive values indicate performance above goal, negative values indicate below goal."
        }
    };
}
...
async function runAllQuartersReport() {
    try {
        ...
        for (let quarterNum = 1; quarterNum <= 4; quarterNum++) {
            ...
            quarterComparisons[`Q${quarterNum}`] = calculateYoYChanges(currentQuarterKPIs, previousQuarterKPIs);
        }
...
async function runReport(periodType = 'mtd') {
    ...
    try {
        // 1. Initialization
        log.header("🍷 MILEA ESTATE VINEYARD - KPI DASHBOARD DATA GENERATOR (OPTIMIZED)");
        log.cyan("=========================================================");
        const startTime = new Date();
...
if (!isSpecificPeriod && !basicPeriods.includes(periodType)) {
    ...
    log.info(`  Specific months: month:1 through month:12 (e.g., month:6 for June)`);
    log.info(`  Specific quarters: quarter:1 through quarter:4 (e.g., quarter:2 for Q2)`);
    ...
    log.info(`  node optimized-kpi-dashboard.js all-quarters  # All quarters analysis`);
    log.info(`  node optimized-kpi-dashboard.js month:6       # June 2025 vs June 2024`);
    log.info(`  node optimized-kpi-dashboard.js quarter:2     # Q2 2025 vs Q2 2024`);
    log.info(`  node optimized-kpi-dashboard.js year:2024     # Full 2024 vs 2023`);
    process.exit(1);
}
