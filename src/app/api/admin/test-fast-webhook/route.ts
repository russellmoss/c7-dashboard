...
export async function POST(request: NextRequest) {
  try {
    console.log("=== TEST FAST WEBHOOK START ===");
    
    // Simulate the same logic as the fast webhook directly
    const fromPhone = "+18457073347";
    const messageBody = "This is a test message from the fast webhook!";
    ...
    console.log("=== TEST FAST WEBHOOK END ===");

    return NextResponse.json({ 
      success: true,
      message: "Fast webhook test completed successfully",
      subscriber: subscriber.name,
      phoneNumber: subscriber.smsCoaching?.phoneNumber
    });
  } catch (error) {
    console.error("Error in test fast webhook:", error);
    return NextResponse.json(
      { error: "Failed to test fast webhook", details: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}
...
export async function GET() {
  return NextResponse.json({
    message: "Fast webhook test endpoint",
    instructions: "POST to this endpoint to test the fast webhook",
    webhookUrl: "/api/webhooks/twilio/sms-fast"
  });
} 