...
export async function POST(request: NextRequest) {
  try {
    ...
    const realKPIData: KPIDashboardData = {
      ...
      periodLabel:
        kpiData.data.current.periodLabel || kpiData.periodType.toUpperCase(),
      ...
      overallMetrics: {
        ...
        wineBottleConversionRate:
          kpiData.data.current.overallMetrics.wineBottleConversionRate,
        clubConversionRate:
          kpiData.data.current.overallMetrics.clubConversionRate,
      },
      yearOverYear: {
    ...
    const mockSubscription = {
      ...
      name: name || "Test User",
      email: email,
      subscribedReports: ["mtd", "qtd", "ytd"] as (
        | "mtd"
        | "qtd"
        | "ytd"
        | "all-quarters"
    ...
    return NextResponse.json(
      { error: "Failed to send beautiful KPI email" },
      { status: 500 },
    );
  }
}
