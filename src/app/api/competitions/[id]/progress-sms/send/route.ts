...
export async function POST(
  ...
  try {
    ...
    if (result.success && result.sentCount > 0) {
      console.log(
        `[API] ✅ Progress SMS sent successfully to ${result.sentCount} subscribers`,
      );
      ...
      console.error(
        `[API] ❌ Progress SMS failed: ${result.errors.join(", ")}`,
      );
    ...
    console.error(
      `[API] Error sending progress SMS for competition ${params.id}:`,
      error,
    );
    return NextResponse.json(
