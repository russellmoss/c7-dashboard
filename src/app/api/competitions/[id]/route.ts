...
export async function GET(
  ...
  try {
    ...
    const hasFinalRankings = competition.finalRankings && competition.finalRankings.length > 0;
    const winners = hasFinalRankings ? competition.finalRankings.filter((r: any) => r.rank <= 3) : [];
    ...
    const competitionWithDetails = {
      ...
      statistics: {
        ...
        averageRank: hasFinalRankings ? 
          competition.finalRankings.reduce((sum: number, r: any) => sum + r.rank, 0) / competition.finalRankings.length : 
          null
      }
    };
...
export async function PUT(
  ...
  try {
    ...
    if (existingCompetition.status === 'completed' || existingCompetition.status === 'archived') {
      return NextResponse.json(
    ...
    if (existingCompetition.status === 'active') {
      const allowedFieldsForActive = [
        'name', 'prizes', 'welcomeMessage', 'progressNotifications', 
        'targetGoals', 'competitionType'
      ];
      const updateFields = Object.keys(body);
      const disallowedFields = updateFields.filter(field => !allowedFieldsForActive.includes(field));
    ...
    if (body.welcomeMessage) {
      ...
      updateBody.welcomeMessage = {
        customText: body.welcomeMessage.customText || existingCompetition.welcomeMessage.customText,
    ...
    if (body.winnerAnnouncement) {
      let winnerAnnouncementScheduledAt = existingCompetition.winnerAnnouncement.scheduledAt;
