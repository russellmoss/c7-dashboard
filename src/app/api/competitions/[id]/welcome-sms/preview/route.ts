...
export async function GET(
  ...
  try {
    ...
    const previews = subscribers.map((subscriber) => ({
      ...
      hasValidPhone:
        subscriber.smsCoaching?.isActive && subscriber.smsCoaching?.phoneNumber,
      phoneNumber: subscriber.smsCoaching?.phoneNumber || "No phone number",
      message: welcomeSmsService.getWelcomeMessagePreview(
        competition,
        subscriber.name,
      ),
    }));
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        statistics: {
          ...
          canSendSms:
            validPhoneSubscribers > 0 && !competition.welcomeMessage.sent,
        },
      },
    });
  } catch (error: any) {
    console.error(
      `[API] Error generating welcome SMS preview for competition ${params.id}:`,
      error,
    );
    return NextResponse.json(
