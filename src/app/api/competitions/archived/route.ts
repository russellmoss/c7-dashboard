...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[API] GET /api/competitions/archived - page: ${page}, limit: ${limit}, search: ${search}`);
    ...
    const stats = archiveStats[0] || { totalArchived: 0, byType: [], byDashboard: [] };
    const typeStats = stats.byType.reduce((acc: any, type: string) => {
      acc[type] = (acc[type] || 0) + 1;
      return acc;
    }, {});
    const dashboardStats = stats.byDashboard.reduce((acc: any, dashboard: string) => {
