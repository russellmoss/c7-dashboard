...
export async function GET(request: NextRequest) {
  try {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      console.log(`[TEST] ✅ Created test competition: ${competition.name} (ID: ${competition._id})`);
    }
    ...
    console.log(`[TEST] ✅ Archive statistics: ${statistics.totalCompetitions} competitions, ${statistics.totalParticipants} participants`);
    
    // Test 3: Search archived competitions
    console.log('[TEST] 🧪 Test 3: Searching archived competitions...');
    const searchResult = await archiveManagementService.searchArchivedCompetitions(
      {},
      { field: 'endDate', direction: 'desc' },
      1,
      10
    );
    console.log(`[TEST] ✅ Search result: ${searchResult.competitions.length} competitions found`);
    
    // Test 4: Test filtering
    console.log('[TEST] 🧪 Test 4: Testing filters...');
    const filteredResult = await archiveManagementService.searchArchivedCompetitions(
      { type: 'bottleConversion' },
      { field: 'name', direction: 'asc' },
      1,
      10
    );
    console.log(`[TEST] ✅ Filtered result: ${filteredResult.competitions.length} bottle conversion competitions`);
    ...
    console.log(`[TEST] ✅ Performance analytics: ${analytics.totalCompetitions} competitions analyzed`);
    
    // Test 6: Archive a competition
    console.log('[TEST] 🧪 Test 6: Archiving a competition...');
    const archiveResult = await archiveManagementService.archiveCompetition(testCompetitions[1]._id.toString());
    console.log(`[TEST] ✅ Archive result: ${archiveResult ? 'Success' : 'Failed'}`);
    
    // Test 7: Restore a competition
    console.log('[TEST] 🧪 Test 7: Restoring a competition...');
    const restoreResult = await archiveManagementService.restoreCompetition(testCompetitions[1]._id.toString());
    console.log(`[TEST] ✅ Restore result: ${restoreResult ? 'Success' : 'Failed'}`);
    
    // Test 8: Get competition details
    console.log('[TEST] 🧪 Test 8: Getting competition details...');
    const competitionDetails = await archiveManagementService.getCompetitionDetails(testCompetitions[0]._id.toString());
    console.log(`[TEST] ✅ Competition details: ${competitionDetails ? competitionDetails.name : 'Not found'}`);
    
    // Test 9: Test API endpoints
    console.log('[TEST] 🧪 Test 9: Testing API endpoints...');
    
    // Test archive competitions API
    const archiveApiResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/archive/competitions?page=1&limit=10`);
    if (archiveApiResponse.ok) {
      const archiveApiData = await archiveApiResponse.json();
      console.log(`[TEST] ✅ Archive competitions API: ${archiveApiData.data.competitions.length} competitions`);
    } else {
      console.log(`[TEST] ⚠️ Archive competitions API failed: ${archiveApiResponse.statusText}`);
    }
    
    // Test archive statistics API
    const statsApiResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/archive/statistics`);
    if (statsApiResponse.ok) {
      const statsApiData = await statsApiResponse.json();
      console.log(`[TEST] ✅ Archive statistics API: ${statsApiData.data.totalCompetitions} total competitions`);
    } else {
      console.log(`[TEST] ⚠️ Archive statistics API failed: ${statsApiResponse.statusText}`);
    }
    
    // Test archive analytics API
    const analyticsApiResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/archive/analytics`);
    if (analyticsApiResponse.ok) {
      const analyticsApiData = await analyticsApiResponse.json();
      console.log(`[TEST] ✅ Archive analytics API: ${analyticsApiData.data.totalCompetitions} competitions analyzed`);
    } else {
      console.log(`[TEST] ⚠️ Archive analytics API failed: ${analyticsApiResponse.statusText}`);
    }
