...
export async function GET(request: NextRequest) {
  try {
    ...
    const testCompetitionData = {
      ...
      welcomeMessage: {
        customText: 'Welcome to the test competition! This tests the admin UI integration.',
        sendAt: null
      },
      enrolledSubscribers: subscribers.map(s => s._id)
    };
    
    const createResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/competitions`, {
      method: 'POST',
    ...
    if (!createResponse.ok) {
      throw new Error(`Failed to create competition: ${createResponse.statusText}`);
    }
    
    const createdCompetition = await createResponse.json();
    const competitionId = createdCompetition.data.competition._id;
    console.log(`[TEST] ✅ Created competition: ${createdCompetition.data.competition.name} (ID: ${competitionId})`);
    
    // Test 2: Fetch competitions list
    console.log('[TEST] 🧪 Test 2: Fetching competitions list...');
    const listResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/competitions?status=draft`);
    const competitionsList = await listResponse.json();
    console.log(`[TEST] ✅ Found ${competitionsList.data.competitions.length} draft competitions`);
    
    // Test 3: Get individual competition details
    console.log('[TEST] 🧪 Test 3: Getting competition details...');
    const detailResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/competitions/${competitionId}`);
    const competitionDetails = await detailResponse.json();
    console.log(`[TEST] ✅ Fetched competition details: ${competitionDetails.data.name}`);
    
    // Test 4: Add notification
    console.log('[TEST] 🧪 Test 4: Adding notification...');
    const notificationResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/competitions/${competitionId}/add-notification`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (notificationResponse.ok) {
      ...
      console.log(`[TEST] ✅ Added notification: ${notificationResult.data.notificationId}`);
    } else {
      console.log(`[TEST] ⚠️ Notification add failed: ${notificationResponse.statusText}`);
    }
    
    // Test 5: Activate competition
    console.log('[TEST] 🧪 Test 5: Activating competition...');
    const activateResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/competitions/${competitionId}/activate`, {
      method: 'POST'
    });
    ...
    const activeResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/competitions?status=active`);
    const activeCompetitions = await activeResponse.json();
    console.log(`[TEST] ✅ Found ${activeCompetitions.data.competitions.length} active competitions`);
    
    // Test 7: Get archived competitions
    console.log('[TEST] 🧪 Test 7: Fetching archived competitions...');
    const archivedResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/competitions/archived`);
    const archivedCompetitions = await archivedResponse.json();
    console.log(`[TEST] ✅ Found ${archivedCompetitions.data.competitions.length} archived competitions`);
    
    // Test 8: Test welcome SMS (if competition is active)
    console.log('[TEST] 🧪 Test 8: Testing welcome SMS...');
    const smsResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/competitions/${competitionId}/welcome-sms/send`, {
      method: 'POST'
    });
