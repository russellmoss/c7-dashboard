...
export async function GET(request: NextRequest) {
  try {
    ...
    const rankingCompetitionData = {
      ...
      welcomeMessage: {
        customText: 'Welcome to our ranking competition! Top 3 performers will win prizes.',
        sendAt: null,
        sent: false,
        sentAt: null
      },
    ...
    const rankingCompetition = await CompetitionModel.create(rankingCompetitionData);
    console.log(`[TEST] ✅ Created ranking competition: ${rankingCompetition.name}`);
    ...
    const targetCompetitionData = {
      ...
      endDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 days from now
      welcomeMessage: {
        customText: 'Welcome to our target competition! Hit the goals to win prizes.',
        sendAt: null,
        sent: false,
        sentAt: null
      },
    ...
    const targetCompetition = await CompetitionModel.create(targetCompetitionData);
    console.log(`[TEST] ✅ Created target competition: ${targetCompetition.name}`);
    
    // Test 3: Test editing active competition
    console.log('[TEST] 🧪 Test 3: Testing edit active competition...');
    await CompetitionModel.findByIdAndUpdate(rankingCompetition._id, { status: 'active' });
    
    const editResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/competitions/${rankingCompetition._id}`, {
      method: 'PUT',
    ...
    if (editResponse.ok) {
      ...
      console.log(`[TEST] ⚠️ Failed to edit active competition: ${editResponse.statusText}`);
    }
    
    // Test 4: Test deleting active competition
    console.log('[TEST] 🧪 Test 4: Testing delete active competition...');
    const deleteResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/competitions/${rankingCompetition._id}`, {
      method: 'DELETE'
    });
    ...
    if (deleteResponse.ok) {
      ...
      console.log(`[TEST] ⚠️ Failed to delete active competition: ${deleteResponse.statusText}`);
    }
