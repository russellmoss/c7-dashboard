...
export async function GET(request: NextRequest) {
  try {
    ...
    const testCompetitionData = {
      ...
      welcomeMessage: {
        customText: 'Welcome to the test competition! This tests the management API.',
      ...
      winnerAnnouncement: {
        scheduledAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000 + 60 * 60 * 1000), // endDate + 1 hour
        sent: false,
        sentAt: null
      },
    ...
    console.log(`[TEST] ✅ Created competition: ${testCompetition.name} (ID: ${testCompetition._id})`);
    
    // Test 2: Get competition details
    console.log('[TEST] 🧪 Test 2: Getting competition details...');
    const fetchedCompetition = await CompetitionModel.findById(testCompetition._id).lean();
    ...
    console.log(`[TEST] ✅ Activated competition: ${activatedCompetition.name}`);
    
    // Test 6: Mark welcome message as sent
    console.log('[TEST] 🧪 Test 6: Marking welcome message as sent...');
    const competitionForSMS = await CompetitionModel.findById(testCompetition._id);
    ...
    const competitionsList = await CompetitionModel.find({ status: 'active' }).lean();
    console.log(`[TEST] ✅ Found ${competitionsList.length} active competitions`);
    
    // Test 8: Get archived competitions (should be empty)
    console.log('[TEST] 🧪 Test 8: Getting archived competitions...');
    const archivedCompetitions = await CompetitionModel.find({ status: 'archived' }).lean();
    console.log(`[TEST] ✅ Found ${archivedCompetitions.length} archived competitions`);
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        competitionDetails: {
          ...
          notificationsCount: updatedCompetition?.progressNotifications?.length || 0
        }
      }
    });
