...
import { calculateCompetitionRankings, getCacheStats } from '@/lib/competition-ranking';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[TEST] 📊 Found ${competitions.length} competitions in database`);
    
    if (competitions.length === 0) {
      console.log('[TEST] ⚠️  No competitions found. Creating a test competition...');
      ...
      if (!kpiData) {
        return NextResponse.json({
          ...
          message: 'Please generate MTD KPI data first using the dashboard refresh button.'
        });
      }
      ...
      const rankings = await calculateCompetitionRankings(testCompetition._id.toString());
      ...
      return NextResponse.json({
        success: true,
        message: 'Test competition created and rankings calculated successfully',
        data: {
          competition: {
      ...
      for (const competition of competitions) {
        ...
        try {
          const rankings = await calculateCompetitionRankings(competition._id.toString());
