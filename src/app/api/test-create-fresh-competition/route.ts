...
export async function POST(request: NextRequest) {
  try {
    console.log(`[API] POST /api/test-create-fresh-competition`);
    ...
    // Create a test competition with scheduled SMS due in the past but NOT sent
    const testCompetition = {
      name: 'Fresh Test Worker Competition',
      ...
      welcomeMessage: {
        customText: 'Welcome to the fresh test competition!',
      ...
      progressNotifications: [
        {
          ...
          sent: false, // NOT sent yet
          sentAt: null,
          customMessage: 'Check your fresh progress!'
        }
      ],
      winnerAnnouncement: {
    ...
    console.log(`[TEST] Created fresh test competition: ${competition._id}`);
    ...
    console.error(`[API] Error in test-create-fresh-competition: ${error.message}`);
    return NextResponse.json(
      { error: error.message },
      { status: 500 }
    );
