...
export async function GET(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' && process.env.SKIP_INTEGRATION_TESTS === 'true') {
      console.log("[API] ⏭️ Skipping progress SMS tests during build (API not available)");
      return NextResponse.json({
        success: true,
        message: "Progress SMS tests skipped during build",
        data: {
          testResults: {
    ...
    console.log(
      `[TEST] ✅ Created test competition: ${testCompetition.name} (ID: ${testCompetition._id})`,
    );

    // Test 2: Validate progress SMS
    console.log("[TEST] 🧪 Test 2: Validating progress SMS...");
    const validation = await progressSmsService.validateProgressSms(
    ...
    console.log(
      "[TEST] 🧪 Test 3: Generating progress message preview with Claude AI...",
    );
    const previewMessage = await progressSmsService.getProgressMessagePreview(
      testCompetition._id.toString(),
      subscribers[0].name,
    );
    console.log(
    ...
    const previewResponse = await fetch(
              `${process.env.NEXTAUTH_URL || process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/competitions/${testCompetition._id}/progress-sms/preview`,
    );
    if (previewResponse.ok) {
      const previewData = await previewResponse.json();
      console.log(
        `[TEST] ✅ Progress SMS preview API working: ${previewData.data.statistics.validPhoneSubscribers} valid phones`,
      );
      console.log(
        `[TEST] 📊 Rankings available: ${previewData.data.rankings.current.length} participants`,
      );
    } else {
      console.log(
        `[TEST] ⚠️ Progress SMS preview API failed: ${previewResponse.statusText}`,
      );
    }

    // Test 5: Test progress SMS sending (if Twilio is configured)
    console.log(
      "[TEST] 🧪 Test 5: Testing progress SMS sending with Claude AI...",
    );
    const smsResult = await progressSmsService.sendProgressSms(
      testCompetition._id.toString(),
      "Keep up the great work! 🍷",
    );
    console.log(
      `[TEST] ✅ Progress SMS sending result: ${smsResult.sentCount} sent, ${smsResult.failedCount} failed`,
    );

    if (smsResult.sentCount > 0) {
      console.log(
        `[TEST] 🎉 Successfully sent ${smsResult.sentCount} Claude AI-generated progress SMS messages!`,
      );
    } else if (smsResult.failedCount > 0) {
      console.log(
        `[TEST] ⚠️ Progress SMS sending failed: ${smsResult.errors.join(", ")}`,
      );
    }
    ...
    try {
      ...
      const claudeResponse =
        await progressSmsService["callClaude"](claudePrompt);
      console.log(
    ...
    console.log(
      `[TEST] ✅ Ranking integration working: ${rankings.rankings.length} rankings calculated`,
    );
    ...
    return NextResponse.json({
      success: true,
      message:
        "Progress SMS implementation with Claude AI test completed successfully",
      data: {
        tests: {
