...
export async function GET(request: NextRequest) {
  try {
    ...
    const testCompetition = await CompetitionModel.create({
      ...
      welcomeMessage: {
        customText: 'Welcome to the test competition! This is a custom welcome message.',
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        progressNotifications: updatedCompetition.progressNotifications.map((n: any) => ({
          id: n.id,
        ...
        methodTests: {
          ...
          notificationMarkedSent: updatedCompetition.progressNotifications.find((n: any) => n.id === 'test-notification-1')?.sent || false,
          welcomeMessageMarkedSent: updatedCompetition.welcomeMessage.sent,
          notificationRemoved: !updatedCompetition.progressNotifications.find((n: any) => n.id === 'test-notification-2')
        }
      }
    });
