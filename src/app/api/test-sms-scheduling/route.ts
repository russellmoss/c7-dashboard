...
export async function GET(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' && process.env.SKIP_INTEGRATION_TESTS === 'true') {
      console.log("[API] ⏭️ Skipping SMS scheduling tests during build (API not available)");
      return NextResponse.json({
        success: true,
        message: "SMS scheduling tests skipped during build",
        data: {
          testResults: {
            competitionCreated: { success: true, message: "Skipped during build" },
            schedulingVerified: { success: true, message: "Skipped during build" },
            apiTest: { success: true, message: "Skipped during build" },
            updateTest: { success: true, message: "Skipped during build" }
          }
        }
      });
    }
    ...
    console.log(
      "[TEST] 🧪 Test 1: Creating competition with SMS scheduling...",
    );
    ...
    const competitionData = {
      ...
      progressNotifications: [
        ...
        {
          scheduledDate: new Date(tomorrow.getTime() + 5 * 24 * 60 * 60 * 1000)
            .toISOString()
            .split("T")[0],
          scheduledTime: "16:00",
      ...
      winnerAnnouncement: {
        scheduledDate: new Date(nextWeek.getTime() + 24 * 60 * 60 * 1000)
          .toISOString()
          .split("T")[0],
        scheduledTime: "10:00",
      },
    ...
    const createdCompetition = await CompetitionModel.findById(
      competition._id,
    ).lean();
    ...
    const schedulingResults = {
      ...
      progressNotifications: {
        ...
        notifications: createdCompetition.progressNotifications.map(
          (n: any) => ({
            id: n.id,
    ...
    const updateData = {
      welcomeMessage: {
        ...
        scheduledDate: new Date(tomorrow.getTime() + 24 * 60 * 60 * 1000)
          .toISOString()
          .split("T")[0],
        scheduledTime: "10:00",
      },
      progressNotifications: [
        {
          scheduledDate: new Date(tomorrow.getTime() + 3 * 24 * 60 * 60 * 1000)
            .toISOString()
            .split("T")[0],
          scheduledTime: "15:00",
    ...
    const updateResponse = await fetch(
      `${process.env.NEXTAUTH_URL || "http://localhost:3000"}/api/competitions/${competition._id}`,
      {
        method: "PUT",
    ...
    if (updateResponse.ok) {
      ...
      console.log(
        `[TEST] ⚠️ Failed to update SMS scheduling: ${updateResponse.statusText}`,
      );
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        features: {
          welcomeMessageScheduling:
            "Date and time scheduling for welcome messages",
          progressNotificationScheduling:
            "Multiple progress notifications with custom dates/times",
          winnerAnnouncementScheduling:
            "Date and time scheduling for winner announcements",
          schedulingUpdates:
            "Ability to update scheduling for active competitions",
