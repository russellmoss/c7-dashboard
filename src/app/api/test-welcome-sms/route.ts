...
export async function GET(request: NextRequest) {
  try {
    ...
    const testCompetitionData = {
      ...
      welcomeMessage: {
        customText:
          "Welcome to our test competition! This is a custom welcome message to test the SMS functionality.",
        sendAt: null,
        sent: false,
        sentAt: null,
      },
    ...
    const validation = await welcomeSmsService.validateWelcomeSms(
    ...
    const previewMessage = await welcomeSmsService.getWelcomeMessagePreview(
      testCompetition,
      subscribers[0].name,
    );
    console.log(
      `[TEST] ✅ Generated preview message for ${subscribers[0].name}:`,
    );
    ...
    const previewResponse = await fetch(
              `${process.env.NEXTAUTH_URL || process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/competitions/${testCompetition._id}/welcome-sms/preview`,
    );
    if (previewResponse.ok) {
      const previewData = await previewResponse.json();
      console.log(
        `[TEST] ✅ SMS preview API working: ${previewData.data.statistics.validPhoneSubscribers} valid phones`,
      );
    } else {
      console.log(
        `[TEST] ⚠️ SMS preview API failed: ${previewResponse.statusText}`,
      );
    }

    // Test 5: Test welcome SMS sending (if Twilio is configured)
    console.log("[TEST] 🧪 Test 5: Testing welcome SMS sending...");
    const smsResult = await welcomeSmsService.sendWelcomeSms(
      testCompetition._id.toString(),
    );
    console.log(
      `[TEST] ✅ SMS sending result: ${smsResult.sentCount} sent, ${smsResult.failedCount} failed`,
    );

    if (smsResult.sentCount > 0) {
      console.log(
        `[TEST] 🎉 Successfully sent ${smsResult.sentCount} welcome SMS messages!`,
      );
    } else if (smsResult.failedCount > 0) {
      console.log(
        `[TEST] ⚠️ SMS sending failed: ${smsResult.errors.join(", ")}`,
      );
    }
    ...
    if (updatedCompetition) {
      console.log(
        `[TEST] ✅ Welcome message sent: ${updatedCompetition.welcomeMessage.sent}`,
      );
    } else {
    ...
    const duplicateResult = await welcomeSmsService.sendWelcomeSms(
      testCompetition._id.toString(),
    );
    console.log(
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        smsFeatures: {
          messageFormatting:
            "Personalized welcome messages with competition details",
