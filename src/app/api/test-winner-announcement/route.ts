...
export async function GET(request: NextRequest) {
  try {
    console.log(
      "[TEST] Testing Winner Announcement SMS Implementation with Claude AI...",
    );

    // Skip tests during build time
    if (process.env.NODE_ENV === 'production' && process.env.SKIP_INTEGRATION_TESTS === 'true') {
      console.log("[API] ⏭️ Skipping winner announcement tests during build (API not available)");
      return NextResponse.json({
        success: true,
        message: "Winner announcement tests skipped during build",
        data: {
          testResults: {
    ...
    console.log(
      `[TEST] 👥 Found ${subscribers.length} subscribers with SMS coaching`,
    );
    ...
    console.log(
      `[TEST] ✅ Created test completed competition: ${testCompetition.name} (ID: ${testCompetition._id})`,
    );

    // Test 2: Validate winner announcement
    console.log("[TEST] 🧪 Test 2: Validating winner announcement...");
    const validation =
      await winnerAnnouncementService.validateWinnerAnnouncement(
        testCompetition._id.toString(),
      );
    console.log(
      `[TEST] ✅ Validation result: ${validation.valid ? "Valid" : "Invalid"}`,
    );
    if (!validation.valid) {
      console.log(
        `[TEST] ⚠️ Validation errors: ${validation.errors.join(", ")}`,
      );
    }

    // Test 3: Generate winner announcement message preview
    console.log(
      "[TEST] 🧪 Test 3: Generating winner announcement message preview with Claude AI...",
    );
    ...
    console.log(
      `[TEST] ✅ Generated Claude AI preview message for ${subscribers[0].name}:`,
    );
    ...
    if (previewResponse.ok) {
      ...
      console.log(
        `[TEST] ✅ Winner announcement preview API working: ${previewData.data.statistics.validPhoneSubscribers} valid phones`,
      );
      console.log(
        `[TEST] 🏆 Winners determined: ${previewData.data.winners.count} winners`,
      );
    } else {
      console.log(
        `[TEST] ⚠️ Winner announcement preview API failed: ${previewResponse.statusText}`,
      );
    }

    // Test 5: Test winner announcement sending (if Twilio is configured)
    console.log(
      "[TEST] 🧪 Test 5: Testing winner announcement sending with Claude AI...",
    );
    const smsResult = await winnerAnnouncementService.sendWinnerAnnouncement(
      testCompetition._id.toString(),
      "Congratulations to all participants! 🍷",
    );
    console.log(
      `[TEST] ✅ Winner announcement sending result: ${smsResult.sentCount} sent, ${smsResult.failedCount} failed`,
    );

    if (smsResult.sentCount > 0) {
      console.log(
        `[TEST] 🎉 Successfully sent ${smsResult.sentCount} Claude AI-generated winner announcement SMS messages!`,
      );
    } else if (smsResult.failedCount > 0) {
      console.log(
        `[TEST] ⚠️ Winner announcement sending failed: ${smsResult.errors.join(", ")}`,
      );
    }
    ...
    try {
      ...
      const claudeResponse =
        await winnerAnnouncementService["callClaude"](claudePrompt);
      console.log(
        `[TEST] ✅ Claude AI integration working: ${claudeResponse.substring(0, 50)}...`,
      );
    } catch (error: any) {
    ...
    console.log(
      `[TEST] ✅ Winner determination working: ${[winners.first, winners.second, winners.third].filter((w) => w !== null).length} winners determined`,
    );
    ...
    console.log(
      `[TEST] ✅ Duplicate send prevention: ${duplicateResult.success ? "Working" : "Failed"}`,
    );
    ...
    return NextResponse.json({
      success: true,
      message:
        "Winner announcement SMS implementation with Claude AI test completed successfully",
      data: {
        tests: {
        ...
        winnerAnnouncementFeatures: {
          ...
          duplicatePrevention:
            "Prevents multiple sends of winner announcements",
        },
        claudeFeatures: {
