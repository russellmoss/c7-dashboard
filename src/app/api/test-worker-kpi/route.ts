...
const scriptPath = join(__dirname, '../../../../scripts/optimized-kpi-dashboard.cjs');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[TEST] Testing KPI generation for ${periodType.toUpperCase()}`);
    ...
    try {
      ...
      const { stdout, stderr } = await execAsync(`node "${scriptPath}" ${periodType}`, {
      ...
      console.log(`[TEST] ✅ ${periodType.toUpperCase()} KPI generation completed successfully`);
      ...
      console.log(`[TEST] ❌ ${periodType.toUpperCase()} KPI generation failed`);
    }
