...
export default function AllQuartersDashboard() {
  ...
  if (loading) {
    return (
      <div className="text-center p-10">Loading All-Quarters Dashboard...</div>
    );
  }
      ...
      <div className="space-y-10">
        {Object.entries(quarters).map(([quarter, qData]: any) => {
          ...
          const staff = qData?.associatePerformance
            ? Object.entries(qData.associatePerformance).map(
                ([name, perf]: any) => ({
                  name,
                  orders: perf.orders,
                  guests: perf.guests,
                  ...
                  wineBottleConversionGoalVariance:
                    perf.wineBottleConversionGoalVariance,
                  clubConversionGoalVariance: perf.clubConversionGoalVariance,
                  aov: perf.aov, // <-- Pass AOV
                }),
              )
            : [];
          const quarterInsights = null; // insights?.[quarter] || insights || null;
          return (
            <div
              key={quarter}
              className="border rounded-lg p-4 bg-white shadow-sm"
            >
              <h2 className="text-xl font-semibold mb-2 text-wine-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                <KPICard
                  title="Total Revenue"
                  value={`$${comparison?.revenue?.current?.toLocaleString()}`}
                  change={comparison?.revenue?.change}
                  icon={<TrendingUp />}
                />
                <KPICard
                  title="Wine Conversion"
                  value={`${comparison?.wineConversionRate?.current}%`}
                  goal={comparison?.wineConversionRate?.goal}
                  goalVariance={comparison?.wineConversionRate?.goalVariance}
                  icon={<Wine />}
                />
                <KPICard
                  title="Club Conversion"
                  value={`${comparison?.clubConversionRate?.current}%`}
                  goal={comparison?.clubConversionRate?.goal}
                  goalVariance={comparison?.clubConversionRate?.goalVariance}
                  icon={<Crown />}
                />
                <KPICard
                  title="Total Guests"
                  value={comparison?.guests?.current?.toLocaleString()}
                  change={comparison?.guests?.change}
                  icon={<Users />}
                />
              </div>
              ...
              {qData?.associatePerformance && (
                <TeamPerformanceAnalysis 
                  associatePerformance={qData.associatePerformance}
                  periodLabel={`${quarter.toUpperCase()} ${new Date().getFullYear()}`}
                />
              )}
            </div>
          );
