...
export function AIInsightsPanel({ insights }: { insights: any }) {
  ...
  const SWOTCard = ({ title, items, icon: Icon, color }: { title: string, items: string[], icon: any, color: string }) => (
  ...
  const StaffList = ({ title, items, color }: { title: string, items: any[], color: string }) => (
    <Card className={`animate-fade-in`}>
      ...
      <CardContent>
        <ul className="space-y-2">
          {items.length === 0 && <li className="text-muted-foreground">None found.</li>}
          {items.map((staff, i) => (
            <li key={i} className="flex flex-col">
              <span className="font-semibold">{staff.name}</span>
              <span className={`text-xs text-${color}-700`}>{staff.reason}</span>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <StaffList title="Staff Praise" items={insights.staffPraise || []} color="green" />
        <StaffList title="Staff Coaching" items={insights.staffCoaching || []} color="red" />
      </div>
        ...
        <CardContent>
          <div className="space-y-3">
            {insights.recommendations.map((rec: string, index: number) => (
              <div key={index} className="flex items-start space-x-3 p-3 bg-muted rounded-lg animate-fade-in">
                <Badge>{index + 1}</Badge>
