...
export class CompetitionAnalyticsService {
   ...
   */
  async getCompetitionAnalytics(filters: AnalyticsFilters = {}): Promise<CompetitionAnalytics> {
    try {
      ...
      const performance = await this.calculatePerformanceAnalytics(competitions, allSubscribers, filters);

      // Calculate effectiveness metrics
      const effectiveness = this.calculateEffectivenessMetrics(competitions);

      // Generate insights
      const insights = this.generateInsights(overview, trends, performance, effectiveness);
      ...
      console.error('[CompetitionAnalyticsService] Error getting analytics:', error);
   ...
   * Calculate overview metrics
   */
  private calculateOverviewMetrics(competitions: any[]): CompetitionAnalytics['overview'] {
    ...
    competitions.forEach(competition => {
      ...
      const totalNotifications = 1 + (competition.progressNotifications?.length || 0) + 1;
    ...
    return {
      ...
      averageCompletionRate: Math.round(totalCompletionRate / competitions.length)
    };
  }
  ...
  private calculateTrends(competitions: any[]): CompetitionAnalytics['trends'] {
    ...
    competitions.forEach(competition => {
      ...
      if (!quarterly.has(quarterKey)) {
        quarterly.set(quarterKey, { competitions: 0, participants: 0, winners: 0 });
      }
  ...
  private async calculatePerformanceAnalytics(
    ...
    // Calculate by type
    const byType = {
      bottleConversion: this.calculateTypeAnalytics(competitions.filter(c => c.type === 'bottleConversion')),
      clubConversion: this.calculateTypeAnalytics(competitions.filter(c => c.type === 'clubConversion')),
      aov: this.calculateTypeAnalytics(competitions.filter(c => c.type === 'aov'))
    };

    // Calculate by dashboard
    const byDashboard = {
      mtd: this.calculateDashboardAnalytics(competitions.filter(c => c.dashboard === 'mtd')),
      qtd: this.calculateDashboardAnalytics(competitions.filter(c => c.dashboard === 'qtd')),
      ytd: this.calculateDashboardAnalytics(competitions.filter(c => c.dashboard === 'ytd'))
    };

    // Calculate by staff
    const byStaff = await this.calculateStaffAnalytics(competitions, subscribers, filters);
   ...
   */
  private calculateTypeAnalytics(competitions: any[]): CompetitionTypeAnalytics {
    if (competitions.length === 0) {
      return {
        count: 0,
        totalParticipants: 0,
    ...
    competitions.forEach(competition => {
      ...
      if (competition.finalRankings?.length > 0) {
        const firstPlace = competition.finalRankings.find((r: any) => r.rank === 1);
        const secondPlace = competition.finalRankings.find((r: any) => r.rank === 2);
        const thirdPlace = competition.finalRankings.find((r: any) => r.rank === 3);
    ...
    const averageParticipants = Math.round(totalParticipants / competitions.length);
    ...
    const averageCompletionRate = Math.round(totalCompletionRate / competitions.length);
    const participationRate = Math.round((totalParticipants / (competitions.length * 10)) * 100); // Assuming max 10 participants per competition
  ...
  private calculateDashboardAnalytics(competitions: any[]): DashboardAnalytics {
    ...
    const averageDuration = Math.round(totalDuration / competitions.length);
    const averageCompletionRate = Math.round(totalCompletionRate / competitions.length);
    const participationRate = Math.round((totalParticipants / (competitions.length * 10)) * 100);
  ...
  private async calculateStaffAnalytics(
    ...
    subscribers.forEach(subscriber => {
      if (filters.staffMember && subscriber.name !== filters.staffMember) return;
    ...
    competitions.forEach(competition => {
      ...
      competition.finalRankings?.forEach((ranking: any) => {
        ...
        // Update average rank
        const currentTotal = (staff.averageRank * (staff.totalParticipations - 1)) + ranking.rank;
    ...
    staffMap.forEach(staff => {
      staff.winRate = staff.totalParticipations > 0 ? Math.round((staff.totalWins / staff.totalParticipations) * 100) : 0;
      staff.participationRate = staff.totalCompetitions > 0 ? Math.round((staff.totalParticipations / staff.totalCompetitions) * 100) : 0;
      staff.averageRank = Math.round(staff.averageRank * 10) / 10;
      staff.bestRank = staff.bestRank === Infinity ? 0 : staff.bestRank;
      staff.performanceTrend = this.calculateStaffTrend(staff.recentPerformance);
      ...
      staff.recentPerformance.forEach(perf => {
        const competition = competitions.find(c => c.name === perf.competitionName);
        if (competition) {
          typeCounts[competition.type] = (typeCounts[competition.type] || 0) + 1;
          dashboardCounts[competition.dashboard] = (dashboardCounts[competition.dashboard] || 0) + 1;
        }
      });
      
      staff.favoriteType = Object.entries(typeCounts).sort(([,a], [,b]) => b - a)[0]?.[0] || '';
      staff.favoriteDashboard = Object.entries(dashboardCounts).sort(([,a], [,b]) => b - a)[0]?.[0] || '';
      ...
      staff.effectivenessScore = Math.min(100, Math.max(0, staff.effectivenessScore));

      // Sort recent performance by date
      staff.recentPerformance.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
   ...
   * Calculate effectiveness metrics
   */
  private calculateEffectivenessMetrics(competitions: any[]): CompetitionAnalytics['effectiveness'] {
    if (competitions.length === 0) {
      return {
        participationRates: { overall: 0, byType: {}, byDashboard: {} },
        completionRates: { welcomeMessage: 0, progressNotifications: 0, winnerAnnouncement: 0, overall: 0 },
        engagementMetrics: { averageRank: 0, rankDistribution: {}, winnerRepeatRate: 0, participationGrowth: 0 }
      };
    }

    // Calculate participation rates
    const totalPossibleParticipants = competitions.length * 10; // Assuming max 10 per competition
    const totalActualParticipants = competitions.reduce((sum, c) => sum + (c.enrolledSubscribers?.length || 0), 0);
    const overallParticipationRate = Math.round((totalActualParticipants / totalPossibleParticipants) * 100);
    ...
    ['bottleConversion', 'clubConversion', 'aov'].forEach(type => {
      ...
      const typeParticipants = typeCompetitions.reduce((sum, c) => sum + (c.enrolledSubscribers?.length || 0), 0);
      const typePossible = typeCompetitions.length * 10;
      participationByType[type] = typePossible > 0 ? Math.round((typeParticipants / typePossible) * 100) : 0;
    });

    ['mtd', 'qtd', 'ytd'].forEach(dashboard => {
      const dashboardCompetitions = competitions.filter(c => c.dashboard === dashboard);
      const dashboardParticipants = dashboardCompetitions.reduce((sum, c) => sum + (c.enrolledSubscribers?.length || 0), 0);
      const dashboardPossible = dashboardCompetitions.length * 10;
      participationByDashboard[dashboard] = dashboardPossible > 0 ? Math.round((dashboardParticipants / dashboardPossible) * 100) : 0;
    });
    ...
    competitions.forEach(competition => {
      ...
      const sentProgress = competition.progressNotifications?.filter((n: any) => n.sent).length || 0;
    ...
    const completionRates = {
      welcomeMessage: Math.round((welcomeMessagesSent / competitions.length) * 100),
      progressNotifications: totalProgressNotifications > 0 ? Math.round((progressNotificationsSent / totalProgressNotifications) * 100) : 0,
      winnerAnnouncement: Math.round((winnerAnnouncementsSent / competitions.length) * 100),
      overall: Math.round(((welcomeMessagesSent + progressNotificationsSent + winnerAnnouncementsSent) / (competitions.length + totalProgressNotifications + competitions.length)) * 100)
    };
    ...
    const averageRank = allRanks.length > 0 ? Math.round((allRanks.reduce((sum, rank) => sum + rank, 0) / allRanks.length) * 10) / 10 : 0;

    // Calculate winner repeat rate (simplified)
    const winnerRepeatRate = Math.round((winnerNames.size / Math.max(1, allRanks.length)) * 100);
  ...
  private generateInsights(
    ...
    if (trends.monthly.length >= 2) {
      ...
      if (recent.competitions > previous.competitions) {
        insights.push({
          ...
          impact: 'medium',
          recommendation: 'Consider maintaining this momentum with regular competition scheduling'
        });
      }
    }
    ...
    if (effectiveness.completionRates.overall < 80) {
      insights.push({
        ...
        impact: 'high',
        recommendation: 'Review SMS scheduling and ensure proper follow-up procedures'
      });
    }
    ...
    if (effectiveness.participationRates.overall < 70) {
      insights.push({
        ...
        impact: 'medium',
        recommendation: 'Consider incentives and better communication to increase participation'
      });
    }
   ...
   */
  private calculateTrend(competitions: any[]): 'increasing' | 'decreasing' | 'stable' {
    if (competitions.length < 2) return 'stable';
    
    const sorted = competitions.sort((a, b) => new Date(a.endDate).getTime() - new Date(b.endDate).getTime());
    const recent = sorted.slice(-Math.ceil(sorted.length / 2));
    const older = sorted.slice(0, Math.ceil(sorted.length / 2));
    
    const recentAvg = recent.reduce((sum, c) => sum + (c.enrolledSubscribers?.length || 0), 0) / recent.length;
    const olderAvg = older.reduce((sum, c) => sum + (c.enrolledSubscribers?.length || 0), 0) / older.length;
   ...
   */
  private calculateStaffTrend(recentPerformance: any[]): 'increasing' | 'decreasing' | 'stable' {
    if (recentPerformance.length < 2) return 'stable';
    
    const sorted = recentPerformance.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    const recent = sorted.slice(-Math.ceil(sorted.length / 2));
    const older = sorted.slice(0, Math.ceil(sorted.length / 2));
    
    const recentAvg = recent.reduce((sum, p) => sum + p.rank, 0) / recent.length;
