...
export interface Competition {
  ...
  targetGoals?: { // New: for target-based competitions
  ...
  finalRankings: { subscriberId: mongoose.Types.ObjectId; rank: number; value: number; name: string }[];
  createdAt?: Date;
  updatedAt?: Date;
}
...
const CompetitionSchema = new Schema<Competition>({
  ...
  type: { type: String, enum: ['bottleConversion', 'clubConversion', 'aov'], required: true },
  competitionType: { type: String, enum: ['ranking', 'target'], required: true, default: 'ranking' },
  ...
  status: { type: String, enum: ['draft', 'active', 'completed', 'archived'], required: true },
  finalRankings: [
    {
...
CompetitionSchema.methods.addProgressNotification = function(scheduledAt: Date) {
...
CompetitionSchema.methods.removeProgressNotification = function(notificationId: string) {
  this.progressNotifications = this.progressNotifications.filter(
    (notification: any) => notification.id !== notificationId
  );
};

CompetitionSchema.methods.markNotificationSent = function(notificationId: string) {
  const notification = this.progressNotifications.find((n: any) => n.id === notificationId);
  if (notification) {
...
const goalField = (min: number, max: number, isAOV = false) => ({
  value: {
    type: Number,
    min: isAOV ? 0 : min,
    max: isAOV ? undefined : max,
    validate: isAOV ? { validator: (v: number | undefined) => v == null || v >= 0, message: 'AOV must be positive' } : undefined,
  },
  enabled: { type: Boolean, default: false },
});
...
export const CompetitionModel: Model<Competition> = mongoose.models.Competition || mongoose.model('Competition', CompetitionSchema);
