...
export class WinnerAnnouncementService {
   ...
   */
  async sendWinnerAnnouncement(competitionId: string, customMessage?: string): Promise<WinnerAnnouncementResult> {
    try {
      console.log(`[WinnerAnnouncementService] Starting winner announcement for competition: ${competitionId}`);
      ...
      if (competition.status !== 'completed') {
        throw new Error('Can only send winner announcements for completed competitions');
      }

      // Check if winner announcement has already been sent
      if (competition.winnerAnnouncement.sent) {
        throw new Error('Winner announcement has already been sent for this competition');
      }
      ...
      console.log(`[WinnerAnnouncementService] Found ${subscribers.length} subscribers with valid phone numbers`);
      ...
      if (results.sentCount > 0) {
        ...
        console.log(`[WinnerAnnouncementService] ✅ Winner announcement sent to ${results.sentCount} subscribers`);
      }
      ...
      console.error('[WinnerAnnouncementService] Error sending winner announcement:', error);
      return {
        success: false,
        sentCount: 0,
        failedCount: 0,
   ...
   */
  private async sendToSubscribers(smsData: WinnerAnnouncementData): Promise<WinnerAnnouncementResult> {
    ...
    for (const subscriber of smsData.enrolledSubscribers) {
      try {
        const message = await this.generateWinnerAnnouncementMessage(smsData, subscriber);
        const success = await this.smsService.sendSms(subscriber.smsCoaching.phoneNumber, message);
        
        if (success) {
          results.sentCount++;
          console.log(`[WinnerAnnouncementService] ✅ Winner announcement sent to ${subscriber.name} (${subscriber.smsCoaching.phoneNumber})`);
        } else {
          results.failedCount++;
          results.errors.push(`Failed to send winner announcement to ${subscriber.name}`);
          console.error(`[WinnerAnnouncementService] ❌ Failed to send winner announcement to ${subscriber.name}`);
        }
      } catch (error: any) {
        results.failedCount++;
        results.errors.push(`Error sending winner announcement to ${subscriber.name}: ${error.message}`);
        console.error(`[WinnerAnnouncementService] ❌ Error sending winner announcement to ${subscriber.name}:`, error);
      }
    }
   ...
   */
  private async generateWinnerAnnouncementMessage(smsData: WinnerAnnouncementData, subscriber: any): Promise<string> {
    try {
      ...
      const winnerPosition = this.getWinnerPosition(smsData.winners, subscriber.name);

      // Generate Claude prompt
      const prompt = this.buildWinnerAnnouncementPrompt(smsData, subscriber, subscriberRanking, isWinner, winnerPosition);

      // Call Claude API
      const claudeResponse = await this.callClaude(prompt);

      // Format the final message
      return this.formatWinnerAnnouncementMessage(smsData, subscriber, subscriberRanking, claudeResponse, isWinner, winnerPosition);

    } catch (error: any) {
      console.error(`[WinnerAnnouncementService] Error generating winner announcement for ${subscriber.name}:`, error);
   ...
   */
  private getWinnerPosition(winners: any, subscriberName: string): string | null {
  ...
  private buildWinnerAnnouncementPrompt(
    ...
    const competitionDuration = Math.ceil((new Date(smsData.endDate).getTime() - new Date(smsData.startDate).getTime()) / (1000 * 60 * 60 * 24));
...
Generate a concise, celebratory winner announcement message:`;
  ...
  private async callClaude(prompt: string): Promise<string> {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 300,
        temperature: 0.8,
        system: 'You are a celebration coach for completed competitions. Generate concise, celebratory SMS messages for winner announcements. Keep messages under 160 characters and use emojis sparingly.',
        messages: [
   ...
   */
  private generateFallbackWinnerMessage(smsData: WinnerAnnouncementData, subscriber: any): string {
    const firstName = subscriber.name.split(' ')[0];
    const isWinner = this.isSubscriberWinner(smsData.winners, subscriber.name);
    const winnerPosition = this.getWinnerPosition(smsData.winners, subscriber.name);
   ...
   */
  private async markWinnerAnnouncementSent(competitionId: string, winners: any): Promise<void> {
    try {
      ...
      if (competition) {
        ...
        console.log(`[WinnerAnnouncementService] ✅ Winner announcement marked as sent for competition: ${competitionId}`);
      }
    } catch (error) {
      console.error('[WinnerAnnouncementService] Error marking winner announcement as sent:', error);
    }
  }
  ...
  async validateWinnerAnnouncement(competitionId: string): Promise<{
    ...
    try {
      ...
      if (competition.status !== 'completed') {
        return { valid: false, errors: ['Can only send winner announcements for completed competitions'] };
      }

      if (competition.winnerAnnouncement.sent) {
        return { valid: false, errors: ['Winner announcement has already been sent'] };
      }

      const rankings = await getCompetitionRankings(competitionId, false);
      if (rankings.rankings.length === 0) {
        return { valid: false, errors: ['No final rankings available for this competition'] };
      }
   ...
   */
  async getWinnerAnnouncementPreview(competitionId: string, subscriberName: string): Promise<string> {
    try {
      ...
      const isWinner = this.isSubscriberWinner(validation.winners, subscriberName);
      const winnerPosition = this.getWinnerPosition(validation.winners, subscriberName);
      ...
      const prompt = this.buildWinnerAnnouncementPrompt({
        ...
        winners: validation.winners,
        enrolledSubscribers: validation.subscribers || []
      }, subscriber, ranking, isWinner, winnerPosition);
      ...
      return this.formatWinnerAnnouncementMessage({
        ...
        winners: validation.winners,
        enrolledSubscribers: validation.subscribers || []
      }, subscriber, ranking, claudeResponse, isWinner, winnerPosition);

    } catch (error: any) {
      console.error(`[WinnerAnnouncementService] Error generating preview for ${subscriberName}:`, error);
