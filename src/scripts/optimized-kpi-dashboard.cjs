  ...
  while (continueLooping && page <= MAX_PAGES) {
    ...
    while (currentTry <= MAX_RETRIES && !success) {
      try {
        if (page % 10 === 0)
          log.cyan(
            `   📊 Progress: Page ${page}, ${allItems.length} items fetched...`,
          );
...
async function fetchAllDataParallel(dateRanges) {
  ...
  log.info(
    `📊 Data Fetching Summary: ${allOrders.length} unique orders, ${clubMemberships.length} club memberships`,
  );
...
async function fetchAllDataForAllQuarters(baseDate = new Date()) {
  ...
  for (const qRanges of quarterRangesToFetch) {
    const [current, previous] = await Promise.all([
      fetchPaginatedData("order", {
        orderPaidDate: `btw:${qRanges.current.start.toISOString().split("T")[0]}|${qRanges.current.end.toISOString().split("T")[0]}`,
      }),
      fetchPaginatedData("order", {
        orderPaidDate: `btw:${qRanges.previous.start.toISOString().split("T")[0]}|${qRanges.previous.end.toISOString().split("T")[0]}`,
      }),
    ]);
    allOrderResults.push(...current, ...previous);
  }
  const earliestDate = quarterRangesToFetch[0]?.previous.start;
  const clubMemberships = earliestDate
    ? await fetchPaginatedData("club-membership", {
  ...
  const fetchDuration = (Date.now() - startTime) / 1000;
  log.success(
    `✅ All-quarters data fetched in ${fetchDuration.toFixed(2)} seconds`,
  );
  return { allOrders, clubMemberships };
}
...
function calculateKPIsForPeriod(
  ...
  for (const name in associatePerformance) {
    ...
    const staffOrders = revenueOrders.filter(
      (order) =>
        order.salesAssociate?.name === name && !isReturnOrRefund(order),
    );
    const staffOrderCount = staffOrders.length;
    const staffRevenue = staffOrders.reduce(
      (sum, order) => sum + getOrderRevenue(order),
      0,
    );
    assoc.aov =
      staffOrderCount > 0 ? round(staffRevenue / 100 / staffOrderCount, 2) : 0;
  }
...
function getDefinitions() {
  return {
    totalRevenue:
      "Total Revenue generated from orders within the selected period.",
    totalOrders: "Total number of orders placed within the selected period.",
    totalClubMemberships:
      "Total number of new club memberships acquired within the selected period.",
    wineBottleConversionRate:
      "Percentage of orders that resulted in a new club membership, specifically for wine bottle purchases.",
    clubConversionRate:
      "Percentage of orders that resulted in a new club membership, regardless of product type.",
  };
}
//endregion

// --- MONGODB SAVE FUNCTION ---
async function saveKPIDataToMongoDB(
  kpiData,
  periodType,
  startDate = null,
  endDate = null,
  ...
  const query = { periodType, year: dataDocument.year };
      ...
      // Upsert the data
      await KPIDataModel.findOneAndUpdate(query, dataDocument, {
        upsert: true,
        new: true,
      });
      ...
      retries++;
      log.error(
        `❌ Error saving to MongoDB (attempt ${retries}): ${error.message}`,
      );
...
async function runAllQuartersReport(testMode = false) {
  ...
  try {
    ...
    for (let qNum = 1; qNum <= 4; qNum++) {
      ...
      const [currentKPIs, previousKPIs] = await Promise.all([
        calculateKPIsForPeriod(
          allOrders,
          clubMemberships,
          qRanges.current.start,
          qRanges.current.end,
          qRanges.current.label,
        ),
        calculateKPIsForPeriod(
          allOrders,
          clubMemberships,
          qRanges.previous.start,
          qRanges.previous.end,
          qRanges.previous.label,
        ),
      ]);
      quarterResults[`Q${qNum}`] = {
        current: currentKPIs,
        previous: previousKPIs,
      };
      quarterComparisons[`Q${qNum}`] = calculateYoYChanges(
        currentKPIs,
        previousKPIs,
      );
    }
    ...
    if (testMode) {
      ...
      const saved = await KPIDataModel.findOne({
        periodType: "all-quarters",
        year: baseDate.getFullYear(),
      }).sort({ createdAt: -1 });
    ...
    log.error(
      "An error occurred during the all-quarters analysis:",
...
async function runReport(periodType = "mtd", testMode = false) {
  ...
  try {
    log.header(
      "🍷 MILEA ESTATE VINEYARD - KPI DASHBOARD DATA GENERATOR (OPTIMIZED + MongoDB)",
    );
    log.cyan("=========================================================");
    ...
    log.step(
      "STEP 3: DISPLAYING SUMMARY & EXPORTING TO JSON" +
        (testMode ? "" : " & MONGODB"),
    );
    if (testMode) {
...
async function runCustomDateRangeReport(
  startDateStr,
  endDateStr,
  testMode = false,
) {
  log.header(
    "🍷 MILEA ESTATE VINEYARD - CUSTOM DATE RANGE KPI DASHBOARD (OPTIMIZED + MongoDB)",
  );
  log.cyan("=========================================================");
  const startTime = new Date();
  ...
  log.info(
    `Custom Date Range: ${startDate.toLocaleDateString()} to ${endDate.toLocaleDateString()}`,
  );
  ...
  const { allOrders, clubMemberships } = await fetchAllDataParallel({
    ...
    previous: {
      ...
      label: `${startDate.toLocaleDateString()} (Same period)`,
    },
  });
  ...
  const [currentKPIs, previousKPIs] = await Promise.all([
    ...
    calculateKPIsForPeriod(
      ...
      `${startDate.toLocaleDateString()} (Same period)`,
    ),
  ]);
...
if (periodType === "custom") {
  ...
  runCustomDateRangeReport(startDate, endDate, testMode)
    .then(() => {
      if (!testMode) {
