...
async function testCompetitionRanking() {
  try {
    ...
    if (competitions.length === 0) {
      ...
      if (!kpiData) {
        console.log('❌ No MTD KPI data found. Please generate KPI data first.');
        return;
      }
      ...
      rankings.rankings.forEach((entry, index) => {
        ...
        const metricDisplay = rankings.competitionType === 'aov' 
          ? `$${entry.metricValue.toFixed(2)}`
          : `${entry.metricValue.toFixed(1)}%`;
        
        console.log(`  ${index + 1}. ${entry.name} - ${metricDisplay} (Rank: ${entry.rank}${tieIndicator})`);
      });
      ...
      const cachedRankings = await calculateCompetitionRankings(testCompetition._id.toString());
