...
function deepCompare(obj1, obj2, pathPrefix = '') {
  ...
  if (typeof obj1 !== typeof obj2) {
    diffs.push(`${pathPrefix}: Type mismatch (${typeof obj1} vs ${typeof obj2})`);
    return diffs;
  }
...
async function validateData(periodType) {
  ...
  const jsonFiles = fs.readdirSync(__dirname)
    .filter(f => f.includes(`C7_KPI_Data_${safePeriodType}`) && f.endsWith('.json'))
    .sort((a, b) => b.localeCompare(a));
  if (!jsonFiles.length) {
    console.error('No JSON files found for comparison');
    return;
  }
  const originalData = JSON.parse(fs.readFileSync(path.join(__dirname, jsonFiles[0]), 'utf8'));
  // Load MongoDB data
